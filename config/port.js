...
if (cluster.isMaster) {
    ...
    const workerPort = process.env.PORT || 5000 + cluster.worker.id;
    const server = http.createServer(app).listen(workerPort, () => {
        logger.info(`Connected to port ${workerPort} on Worker ${process.pid}`);
    });
